    ...
    def test_cells(self):
        ...
        for i in range(self.size):
            for j in range(self.size):
                ...
                self.assertFalse(not cell.get_left().isRemoved() and not cell.get_top().isRemoved(), 'Cell still has both left and top Wall.')

    # check wether the top row has all left walls removed (except entry cell)
    def test_topRow(self):
        self.log.debug("test_topRow")
        for i in range(1, self.size):
            self.assertTrue(self.maze.get_cell(0, i).get_left().isRemoved(), 'Cell in top row still has left Wall.')

    # check wether the left row has all top walls removed (except entry cell)
    def test_leftRow(self):
        self.log.debug("test_leftRow")
        for i in range(1, self.size):
            self.assertTrue(self.maze.get_cell(i, 0).get_top().isRemoved(), 'Cell in left row still has top Wall.')
